	...
	$(document).ready(function(){
		...
		$("#load_more_newest").click(function(){
			$.ajax({
				...
				dataType: 'json',				
				success: function( response ) {
					let resp_length 	= response.length;
					let arrayindexnew	= arrayindex;

					let loadmore = response.splice(arrayindexnew,5);
					...
					let array_berita	= (arrayindex + 1) + arrayindexnew ;
					arrayindex = array_berita;

					if (array_berita != resp_length ){
						$(wrapper2).html(table_wrapper2);
					} else {
						$(wrapper2).html(table_wrapper2);
